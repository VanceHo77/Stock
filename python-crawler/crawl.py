...
def main():
    ...
    #往回爬到dateImport.txt的指定日期
    parser.add_argument('-d', '--util', action='store_true',
        help='往回爬到dateImport.txt的指定日期')
    ...
    if args.back or args.check:
        ...
        last_day = datetime(2016,1,1) if args.back else  - timedelta(10)
        max_error = 5
        error_times = 0
        ...
        fileName = 'dataImport.txt'
        with open(fileName) as f:
            content = f.read().splitlines()
        dateAry = content[0].split('/')
        
        last_day = datetime(int(dateAry[0]),int(dateAry[1]),int(dateAry[2]))
